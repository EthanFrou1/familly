...
export default function DailyMysteryBanner() {
  ...
  if (loading || !state || state.status !== 'inProgress') return null

  return (
    <button
      onClick={() => navigate('/games/daily-mystery')}
      className="flex items-center gap-3 w-full rounded-2xl shadow-sm px-4 py-3.5 text-left transition-opacity bg-gradient-to-r from-primary/10 to-amber-50 active:opacity-70"
    >
