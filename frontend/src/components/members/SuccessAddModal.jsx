...
export default function SuccessAddModal({ member, onClose }) {
  ...
  function getInviteMessage() {
    ...
    return `${greeting}Créez votre compte en cliquant sur ce lien :\n${inviteLink}`
  }
  ...
  function handleWhatsApp() {
    const waNumber = member.whatsappNumber
      ? member.whatsappNumber.replace(/[\s\-().]/g, '').replace(/^\+/, '')
      : ''
    const url = waNumber
      ? `https://wa.me/${waNumber}?text=${encodeURIComponent(getInviteMessage())}`
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className="flex items-center justify-center gap-2 rounded-xl bg-[#25D366] py-2.5 text-xs font-semibold text-white active:brightness-90"
                  >
                  ...
                  <button
                    onClick={handleShare}
                    className="flex items-center justify-center gap-2 rounded-xl bg-gray-100 py-2.5 text-xs font-semibold text-gray-700 active:bg-gray-200"
                  >
