...
export default function DailyMysteryHero({ unlocked, minMembers }) {
  ...
  return (
    <div className={`relative rounded-3xl bg-dark shadow-lg shadow-black/10 overflow-hidden p-5 transition-opacity ${finished ? 'opacity-50' : ''}`}>
      ...
      {unlocked && finished && (
        <>
          ...
          <button
            ...
            className="w-full mt-2 rounded-lg py-2 text-center text-xs font-bold text-white/70 active:text-white"
          >
            Voir le classement
          </button>
        </>
