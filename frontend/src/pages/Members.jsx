...
export default function Members() {
  ...
  const [sortMode, setSortMode] = useState('alpha') // 'alpha' | 'oldest' | 'youngest'
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      result = result.filter(m => matchesSearch(`${m.firstName} ${m.lastName}`, search))
    }
    if (aliveFilter === 'alive') result = result.filter(m => m.isAlive !== false)
    else if (aliveFilter === 'dead') result = result.filter(m => m.isAlive === false)
  ...
  }, [members, search, familyFilter, aliveFilter, sortMode])

  // Group by first letter (only in alpha mode)
    ...
    <div className="flex flex-col h-full bg-gray-50 animate-fade-in">
      ...
      <div className="bg-dark px-5 pt-10 pb-4 shrink-0">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => setSortMode(m => m === 'alpha' ? 'oldest' : m === 'oldest' ? 'youngest' : 'alpha')}
              title="Changer le tri"
              className="w-[100px] justify-center flex items-center gap-1.5 rounded-xl bg-white/10 border border-white/10 px-3 py-2 text-xs font-semibold text-white active:bg-white/20"
            >
              <svg className="h-3.5 w-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M3 7h18M7 12h10M11 17h2" />
              </svg>
              {sortMode === 'alpha' ? 'A–Z' : sortMode === 'oldest' ? 'Plus âgé' : 'Plus jeune'}
            </button>
            <button
              onClick={() => setShowExportSheet(true)}
              title="Agendas — exporter les anniversaires"
              className="flex items-center justify-center rounded-xl bg-white/10 border border-white/10 p-2 text-white active:bg-white/20"
            >
      ...
      <div className="bg-dark pb-2 shrink-0">
        <div className="flex gap-2 px-4 py-2 overflow-x-auto scrollbar-none">
          ...
          <Chip
            label="Vivants"
            active={aliveFilter === 'alive'}
            onClick={() => setAliveFilter(aliveFilter === 'alive' ? 'all' : 'alive')}
          />
          <Chip
            label="Décédés"
            active={aliveFilter === 'dead'}
            onClick={() => setAliveFilter(aliveFilter === 'dead' ? 'all' : 'dead')}
          />
        </div>
      </div>
          ...
          <div className="mx-4 mt-4 rounded-2xl bg-white shadow-sm overflow-hidden">
            {filtered.map((m, idx) => (
              <MemberRow
                ...
                color={m.familyId ? familyColorMap[m.familyId] : null}
...
function MemberRow({ m, idx, total, color, onClick }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-3 w-full px-4 py-3 text-left active:bg-gray-50 ${idx < total - 1 ? 'border-b border-gray-50' : ''}`}
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-gray-400 truncate">
          {[age != null && `${age} ans`, m.city].filter(Boolean).join(' · ') || 'Aucune info'}
        </p>
      </div>
      {color && m.familyName ? (
        <span
