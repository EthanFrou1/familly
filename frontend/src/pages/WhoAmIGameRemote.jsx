...
export default function WhoAmIGameRemote() {
  ...
  const suggestions = !submitted && !reveal && guessSearch.trim()
    ? members.filter(m => matchesSearch(`${m.firstName} ${m.lastName}`, guessSearch)).slice(0, 6)
    : []
  ...
  if (step === 'menu') {
    return (
      <div className="overflow-y-auto h-full bg-gray-50 pb-24">
        <GameHeader title="Qui suis-je ?" onBack={() => navigate('/games')} />
    ...
    return (
      <div className="overflow-y-auto h-full bg-gray-50 pb-24">
        ...
        <div className="px-4 mt-6 flex flex-col items-center">
          ...
          {isHost && <ChallengeButton gameType="whoami" roomCode={roomCode} excludedMemberIds={players.map(p => p.memberId)} />}
          ...
          {!reveal && (
            <div className="pt-2">
              <input
                ...
                disabled={submitted}
                placeholder="Qui est-ce ? Tape un nom…"
                className="w-full rounded-xl bg-white shadow-sm px-4 py-3 text-sm font-medium text-gray-800 focus:outline-none disabled:opacity-60"
              />
              {suggestions.length > 0 && (
                <ul className="mt-2 rounded-xl border border-gray-200 bg-white overflow-hidden max-h-48 overflow-y-auto">
                  {suggestions.map(m => (
                    <li key={m.id}>
                      <button
                        onClick={() => handleGuess(m.id)}
                        className="w-full flex items-center gap-2 px-4 py-2.5 text-left text-sm text-gray-700 hover:bg-gray-50 active:bg-primary/10"
                      >
              ...
              {submitted && (
                <p className="text-xs text-gray-400 text-center mt-3">Réponse envoyée, en attente des autres…</p>
              )}
            </div>
          )}
        </div>
